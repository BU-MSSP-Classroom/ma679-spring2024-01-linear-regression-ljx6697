...
```{r}
variables <- c("Lot Area", "Year Built", "Year Remod/Add", "Total Bsmt SF",
               "1st Flr SF", "2nd Flr SF", "Low Qual Fin SF", "Gr Liv Area",
               "Full Bath", "Half Bath", "Bsmt Full Bath", "Bsmt Half Bath",
               "TotRms AbvGrd", "Garage Area", "Wood Deck SF",
               "Open Porch SF", "Pool Area")

correlations <- numeric(length(variables))

for (i in 1:length(variables)) {
  correlations[i] <- cor(ames_raw[[variables[i]]], ames_raw$SalePrice, use = "pairwise.complete.obs")
}

result <- data.frame(Variable = variables, Correlation_with_SalePrice = correlations)
print(result)
...
Your code:
```{r,echo=TRUE}
lmfit_2008_it <- lm(SalePrice~`Gr Liv Area` * `1st Flr SF`  , ames_raw_2008) 

pred_2009_it<-predict(lmfit_2008_it,newdata=ames_raw_2009)
calc_loss(pred_2009,ames_raw_2009$SalePrice)
